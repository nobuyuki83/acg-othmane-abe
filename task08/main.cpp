...
int main() {
  ...
  Eigen::SparseLU< Eigen::SparseMatrix<double> > solver;  // sparse linear system solver
  ...
  {
    const Eigen::SparseMatrix<double> sparse = matrix_laplacian.transpose()*matrix_laplacian + penalty_coeff*matrix_penalty.transpose()*matrix_penalty; // write something here // 
  ...
  Eigen::MatrixXd vtx_xyz_def; // deformed xyz coordinates of vertices
  while (!glfwWindowShouldClose(viewer.window)) {
    ...
    Eigen::MatrixXd rhs0 = matrix_laplacian.transpose()*matrix_laplacian*vtx_xyz_ref + penalty_coeff*matrix_penalty.transpose()*matrix_penalty*vtx_xyz_def;// write some code to compute rhs vector // maybe  matrix_laplacian * vtx_xyz_ref || penalty_coeff * matrix_penalty
