    ...
    const double nrm[3])  //
{
  ...
  const auto ksi1 = dfm2::MyERand48<double>(Xi);
  const auto ksi2 = dfm2::MyERand48<double>(Xi);
  double phi = 2*3.1415*ksi2;
  double z = sqrt(ksi1);
  double theta = acos(z);
  ...
  double cos_ = nrm[0]*dir[0] + nrm[1]*dir[1] + nrm[2]*dir[2]; // cosine weight
  return cos_*2;  
